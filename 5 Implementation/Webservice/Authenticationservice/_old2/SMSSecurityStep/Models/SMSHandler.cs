    ...
    public class SMSHandler
    {
        ...
        public bool send(string number, string text)
        {
            try
            {
                ...
                /*String message = new com.aspsms.webservice.ASPSMSX2().SendSimpleTextSMS(userkey, password, number, orgi, text);
                if (!message.Equals("StatusCode:1"))
                {
                    throw new NotSupportedException("SMS Failure:" + new com.aspsms.webservice.ASPSMSX2().GetStatusCodeDescription(message));
                }*/
                return true;
            }
            catch (Exception ex)
